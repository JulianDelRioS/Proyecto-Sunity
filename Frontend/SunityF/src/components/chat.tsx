...
const Chat: React.FC = () => {
  ...
  useEffect(() => {
    const init = async () => {
      try {
        ...
        // Mapeamos las propiedades del backend a nuestra interfaz
        setUser({
          google_id: data.user.id,
          nombre: data.user.name,
          foto_perfil: undefined, // o data.user.foto_perfil si existe
        });
  ...
  const cargarMensajes = async (otro_id: string) => {
    if (!user || escribiendo) return; // esperamos a que user esté definido
    try {
