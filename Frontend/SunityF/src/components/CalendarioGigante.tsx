...
const CalendarioGigante: React.FC = () => {
  ...
  const handleEventClick = (clickInfo: EventClickArg) => {
    const evento = eventos.find((e) => e.id === clickInfo.event.id) ?? null;
    setEventoSeleccionado(evento);

    if (evento) {
      fetchParticipantes(evento.id);
    }
  };
  ...
  const fetchParticipantes = async (eventoId: string) => {
    try {
      ...
      const anfitrion: Participante = {
        ...
        email: data.evento.anfitrion.email,
        telefono: data.evento.anfitrion.telefono,
        foto_perfil: data.evento.anfitrion.foto_perfil
      };

      setParticipantesEvento([anfitrion, ...data.participantes]);
    } catch (err: any) {
              ...
              {participantesEvento.length > 0 && (
                <div className="modal-participantes">
                  ...
                  <div className="participantes-list">
                    {participantesEvento.map((participante, index) => (
                      <div 
                      ...
                      >
                        <img 
                          src={participante.foto_perfil 
                                ? `http://localhost:8000${participante.foto_perfil}` 
                                : "/default-profile.png"} 
